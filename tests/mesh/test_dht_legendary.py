...
class TestDHTLegendary:
    ...
    @pytest.mark.asyncio
    async def test_surgical_saturation_legendary(self):
        ...
        with patch("warm_logic.kernel.mesh.dht.rust_loader.HAS_RUST_CORE", False):
            ...
            with patch("warm_logic.kernel.mesh.dht.K_PARAM", 1):
                ...
                await rt.update(c2, dht=d)  # Move to end (Hits 223-224)

                rt.buckets[0].contacts = [c1]
                d.ping = AsyncMock(return_value=False)
                await rt.update(c2, dht=d)  # Evict (Hits 228)
        ...
        with patch("warm_logic_rs.RustZKProofGenerator") as zk:
            ...
            # USE hasattr check to avoid 7140 failure if storage is None
            if dht.storage:
                dht.storage.put = MagicMock(side_effect=Exception("io"))
            p.handle_store_value_request(
                {"key": "k", "value": "v", "zk_proof": "p", "commitment": "c"}, ("1", 1)
            )
