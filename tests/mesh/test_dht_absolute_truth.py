...
class TestDHTAbsoluteTruth:
    ...
    async def test_surgical_annihilation(self):
        ...
        with patch("warm_logic.kernel.mesh.dht.rust_loader.HAS_RUST_CORE", False):
            ...
            # Use a revoked node ID that is DIFFERENT from local_id
            rev_id = b"\x33" * 32
            rt.revoked_nodes.add(rev_id)
            await rt.update(Contact(rev_id, "1", 1))  # 117-120

            # Binding error paths (Correct line numbers: 123, 125, 136, 146)
            await rt.update(
            ...
            with patch("warm_logic.kernel.mesh.dht.K_PARAM", 1):
                ...
                await rt.update(self.create_pqc_contact("C_LOCKED"), dht=d)  # 216
        ...
        with patch.object(rt_r, "_use_rust", True):
            ...
            try:
                await rt_r.update(c0)  # 186
            except:
                pass
            rt_r._rust_table.find_closest.side_effect = Exception("err")
            try:
                rt_r.find_neighbors(node_id)  # 236
            except:
                pass
        ...
        with patch("warm_logic.kernel.mesh.dht.rust_loader.HAS_RUST_CORE", True):
            with patch("warm_logic.kernel.mesh.dht.rust_loader.load_rust_core") as lrc:
                ...
                try:
                    RoutingTable(node_id).get_all_contacts()  # 277, 287-308
                except:
                    pass
        ...
        with patch("warm_logic.kernel.mesh.dht.create_transport") as ct:
            ...
            with patch(
                ...
                try:
                    await d1.start(enable_nat_discovery=True)  # 389-390
                except:
                    pass
        ...
        with (
            ...
            await d2.bootstrap()  # 448-449
        with patch("os.path.exists", return_value=False):
            await d2.bootstrap()  # 452-453

        with patch.object(d2.routing, "find_neighbors", return_value=[]):
            await d2.iterative_find_node(node_id)  # 448-449 (Search side)
        d2.rpc_call = AsyncMock(
            side_effect=[{"type": "NODES", "nodes": []}, Exception("fail")]
        )
        try:
            await d2.iterative_find_node(node_id)  # 452-453, 485, 523
        except:
            pass
        ...
        p.handle_find_node({"target_id": node_id.hex()}, ("1", 1))  # 822
        ...
        p.handle_patch_request({"target_hash": "h"}, ("1", 1))  # 1073-1093
