#!/usr/bin/env python3
"""
Plot the direct pre/post effect of the PyO3 Vec<u8> fast path:
- stock PyO3 (crates.io) telemetry vs
- patched PyO3 (vendored) telemetry

This is stronger evidence than treating a modeled "sequence baseline" as a proxy.
"""
...
def load_aggregate(path: Path) -> dict[tuple[str, int], dict]:
    ...
    return {(row["path"], int(row["size_bytes"])): row for row in data["aggregate"]}
...
def _render_plot(
    *,
    ...
    palette = {
        "Vec<u8> arg (stock PyO3)": "#dc2626",  # red
        "Vec<u8> arg (patched PyO3)": "#16a34a",  # green
...
def main() -> None:
    ...
    parser.add_argument(
        "--stock",
        default="out/bridge_eval/bridge_eval_v3_stock_pyo3/full_telemetry.json",
        help="stock telemetry json produced by eval_bridge_v3.py",
    )
    parser.add_argument(
        "--patched",
        default="out/bridge_eval/bridge_eval_v3_pyo3_patch/full_telemetry.json",
        help="patched telemetry json produced by eval_bridge_v3.py",
    )
    parser.add_argument(
        "--stat",
        choices=["p50", "p99"],
        default="p50",
        help="which latency statistic to plot (from telemetry aggregate rows)",
    )
    parser.add_argument(
        "--out",
        default="src/warm_logic/docs/papers/09_boundary_elimination/figures/fig_vec_u8_stock_vs_patched.svg",
    )
    args = parser.parse_args()

    stock = load_aggregate(Path(args.stock))
    patched = load_aggregate(Path(args.patched))
    ...
    sizes = sorted(
        s for s in ({s for (_p, s) in stock.keys()} & {s for (_p, s) in patched.keys()}) if s > 0
    )
    if not sizes:
        raise SystemExit("no overlapping sizes between inputs")

    stat_key = "p50_median" if args.stat == "p50" else "p99_median"

    # Use the shared baseline from the patched run (explicit contiguous copy).
    series = {
        "Vec<u8> arg (stock PyO3)": [
            (s, float(stock[("Copy (Vec<u8> arg)", s)][stat_key])) for s in sizes
        ],
        "Vec<u8> arg (patched PyO3)": [
            (s, float(patched[("Copy (Vec<u8> arg)", s)][stat_key])) for s in sizes
        ],
        "Copy (PyBytes to_vec) baseline": [
            (s, float(patched[("Copy (PyBytes to_vec)", s)][stat_key])) for s in sizes
        ],
    }

    _render_plot(
        title="Vec<u8> Arg Conversion: Stock vs Patched PyO3",
        subtitle=f"{args.stat} median across repeats; corrected by empty-loop subtraction; log-log",
